...
class ProductService {
  ...
  // Método para crear un producto, primero validar que no exista otro con el mismo nombre
  static async create(productData) {
    // Evitar nombres duplicados
    const existingProduct = await ProductRepository.findByName(productData.name);
    if (existingProduct) {
      throw new AppError(ERROR_CODES.VALIDATION_ERROR, 'Ya existe un producto registrado con ese nombre.');
    }
  ...
  static async update(id, data) {
    ...
    if (data.name && data.name !== product.name) {
      ...
      if (duplicate) {
        throw new AppError(ERROR_CODES.VALIDATION_ERROR, 'Ya existe otro producto registrado con ese nombre.');
      }
    }
